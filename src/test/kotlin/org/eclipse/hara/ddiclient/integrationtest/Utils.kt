...
object ManagementClient {
    ...
    fun newInstance(url: String): ManagementApi {
        return object : ManagementApi {
            ...
            override suspend fun getTargetActionStatusAsync(
                auth: String,
                targetId: String,
                actionId: Int
            ): ActionStatus {
...
object TestUtils {
    ...
    val tenantNameToLower = tenantName.lowercase()
    ...
    val directoryDataProvider = object : DirectoryForArtifactsProvider {
        override fun directoryForArtifacts(): File = File(
            downloadRootDirPath
        )
    }
    ...
    val test1Artifact = Updater.SwModule.Artifact(
        "test1", Updater.Hashes(
            "",
            md5OfFileNamed("test1")
        ), 0
    )
    val test2Artifact = Updater.SwModule.Artifact(
        "test2", Updater.Hashes(
            "",
            md5OfFileNamed("test2")
        ), 0
    )
    val test3Artifact = Updater.SwModule.Artifact(
        "test3", Updater.Hashes(
            "",
            md5OfFileNamed("test3")
        ), 0
    )
    val test4Artifact = Updater.SwModule.Artifact(
        "test4", Updater.Hashes(
            "",

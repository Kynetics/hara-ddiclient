...
interface MessageListener {
    ...
    sealed class Message(val description: String) {
        ...
        sealed class State(description: String) : Message(description) {
            data class Downloading(val artifacts: List<Artifact>) :
                State("Client is downloading artifacts from server") {
        ...
        sealed class Event(description: String) : Message(description) {
            ...
            data class DownloadProgress(val fileName: String, val percentage: Double = 0.0) :
                Event("Percent of file downloaded")

            object AllFilesDownloaded : Event("All file needed are downloaded")
            data class UpdateFinished(val successApply: Boolean, val details: List<String>) :
                Event("The update is finished")
